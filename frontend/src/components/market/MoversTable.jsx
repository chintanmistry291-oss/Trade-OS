...
function MoverSection({ title, rows, up, onSelect }) {
  return (
    <div className="card-wine overflow-hidden" data-testid={up ? "gainers-section" : "losers-section"}>
      ...
      <table className="tl">
        <thead>
          <tr><th>Symbol</th><th>Price</th><th style={{ textAlign: "right" }}>Change</th></tr>
        </thead>
        <tbody>
          {rows.map(r => (
            <tr key={r.symbol} onClick={() => onSelect(r.symbol)}
                style={{ cursor: "pointer" }} data-testid={`mover-${r.symbol}`}>
              <td>
                <div className="font-semibold text-[13px]">{r.symbol.replace(".NS", "")}</div>
                <div className="text-[10.5px]" style={{ color: "var(--inkSoft)" }}>{r.name?.slice(0, 28)}</div>
              </td>
...
export default function MoversTable({ movers, onSelect }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      <MoverSection title="Top Gainers" rows={movers.gainers} up onSelect={onSelect} />
      <MoverSection title="Top Losers" rows={movers.losers} up={false} onSelect={onSelect} />
    </div>
  );
}
